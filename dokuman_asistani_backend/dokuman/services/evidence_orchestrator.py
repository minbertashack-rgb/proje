...
def _response_safe_hit(hit: dict[str, Any]) -> dict[str, Any]:
    ...
    observability = derive_hit_observability(item)
    safe["komsu_var_mi"] = bool(item.get("komsu_var_mi")) if item.get("komsu_var_mi") is not None else observability["komsu_var_mi"]
    safe["heading_path_uyumlu_mu"] = (
        bool(item.get("heading_path_uyumlu_mu"))
        if item.get("heading_path_uyumlu_mu") is not None
        else observability["heading_path_uyumlu_mu"]
    )
...
def _derive_answer_decision(
    *,
    ...
    should_abstain = bool(abstention_uygulandi_mi)
...
def prepare_answer_evidence(
    ...
    *,
    ...
    doc_chunk_indexes: dict[int, list[int]] = {}
    for raw in evidences or []:
        if not isinstance(raw, dict):
            continue
        doc_id = raw.get("dokuman_id")
        chunk_index = raw.get("chunk_index")
        try:
            if doc_id in (None, "") or chunk_index in (None, ""):
                continue
            clean_doc_id = int(doc_id)
            clean_chunk_index = int(chunk_index)
        except Exception:
            continue
        doc_chunk_indexes.setdefault(clean_doc_id, []).append(clean_chunk_index)
    ...
    for index, raw in enumerate(evidences, start=1):
        ...
        if hit.get("komsu_var_mi") is None:
            try:
                clean_doc_id = int(hit.get("dokuman_id")) if hit.get("dokuman_id") not in (None, "") else None
                clean_chunk_index = int(hit.get("chunk_index")) if hit.get("chunk_index") not in (None, "") else None
            except Exception:
                clean_doc_id = None
                clean_chunk_index = None
            if clean_doc_id is not None and clean_chunk_index is not None:
                same_doc_indexes = doc_chunk_indexes.get(clean_doc_id, [])
                hit["komsu_var_mi"] = any(
                    other_index != clean_chunk_index and abs(other_index - clean_chunk_index) <= 1
                    for other_index in same_doc_indexes
                )
        hit.update(derive_hit_observability(hit, query=question))
        prepared.append(hit)
...
def _build_evidence_selection_summary(
    *,
    ...
    prepared_hits = list(prepared_hits or canonical_hits or [])
    prepared_observability = [derive_hit_observability(hit) for hit in prepared_hits]
    selected_observability = [derive_hit_observability(hit) for hit in selected_hits]
    return {
        ...
        "komsu_destekli_hit_sayisi": sum(1 for item in prepared_observability if item["komsu_var_mi"]),
        "heading_uyumlu_hit_sayisi": sum(1 for item in prepared_observability if item["heading_path_uyumlu_mu"]),
        "secilen_komsu_destekli_hit_sayisi": sum(1 for item in selected_observability if item["komsu_var_mi"]),
        "secilen_heading_uyumlu_hit_sayisi": sum(1 for item in selected_observability if item["heading_path_uyumlu_mu"]),
    }
...
def orchestrate_evidence_selection(
    ...
    *,
    ...
    evidence_secim_ozeti = _build_evidence_selection_summary(
        ...
        prepared_hits=kanit_meta["kanitlar"],
        selected_hits=secilen_kanitlar,
        forced_parca_idleri=forced_parca_idleri,
