...
def _safe_token_overlap_ratio(query_tokens: list[str], text: str) -> float:
    if not query_tokens or not text:
        return 0.0
    text_tokens = set(_rag_tokens(text))
    if not text_tokens:
        return 0.0
    hit_count = sum(1 for token in set(query_tokens) if token in text_tokens)
    return round(hit_count / max(1, len(set(query_tokens))), 3)


def derive_hit_observability(hit, *, query: str = "") -> dict[str, bool]:
    if not isinstance(hit, dict):
        hit = normalize_retrieval_hit(hit)
    explicit_komsu = hit.get("komsu_var_mi")
    explicit_heading = hit.get("heading_path_uyumlu_mu")

    rerank_meta = hit.get("_rerank") if isinstance(hit.get("_rerank"), dict) else {}
    query_tokens = _rag_tokens(query)
    path_text = _hit_path(hit)
    addr_text = _hit_addr(hit)
    path_match = _coerce_float(
        rerank_meta.get("path_match", rerank_meta.get("adres_terim_kapsama_orani")),
        default=0.0,
    )
    if path_match <= 0.0 and query_tokens:
        path_match = max(
            _safe_token_overlap_ratio(query_tokens, path_text),
            _safe_token_overlap_ratio(query_tokens, addr_text),
        )
    exact_path_match = bool(
        rerank_meta.get("exact_path_match")
        or rerank_meta.get("tam_adres_eslesmesi_var_mi")
    )
    doc_support_hits = _coerce_int(
        rerank_meta.get("doc_support_hits", rerank_meta.get("ayni_dokuman_destek_hit_sayisi"))
    )
    locality_bonus = _coerce_float(rerank_meta.get("locality_bonus"), default=0.0)

    return {
        "komsu_var_mi": (
            bool(explicit_komsu)
            if explicit_komsu is not None
            else bool(doc_support_hits and int(doc_support_hits) > 0) or locality_bonus > 0.0
        ),
        "heading_path_uyumlu_mu": (
            bool(explicit_heading)
            if explicit_heading is not None
            else bool(path_text) and (exact_path_match or path_match >= 0.25)
        ),
    }
...
def normalize_retrieval_hit(hit, *, retrieval_kaynagi: str = "rag.semantic", varsayilan_dokuman_id=None):
    if isinstance(hit, dict):
        raw_dokuman_id = hit.get("dokuman_id")
        if raw_dokuman_id in (None, ""):
            raw_dokuman_id = hit.get("doc_id", varsayilan_dokuman_id)
        return build_retrieval_hit(
            parca_id=hit.get("parca_id") or hit.get("id"),
            dokuman_id=raw_dokuman_id,
